...
def validate_other_name_list(l, enumeration_type, basic):
    ...
    for d in l:
        ...
        # Type II
        if enumeration_type == "NPI-2" and str(d.get('code', '')) not in ORGANIZATION_OTHER_NAME_CHOICES:
        ...
        #Type 1
        if enumeration_type == "NPI-1" and sole_proprietor ==  "NO" and str(d.get('code', '')) not in INDIVIDUAL_OTHER_NAME_CHOICES:
            error = "other_name %s: %s other_name for individuals is not in %s" % (i, d.get('code'), INDIVIDUAL_OTHER_NAME_CHOICES)
            errors.append(error)
            
            
        if enumeration_type == "NPI-1" and sole_proprietor ==  "NO" and not str(d.get('first_name', '')):   
            error = "other_name %s: first_name required for NPI-1." % (i)
            errors.append(error)
            

        if enumeration_type == "NPI-1" and sole_proprietor ==  "NO" and not str(d.get('last_name', '')):   
            error = "other_name %s: last_name required for NPI-2" % (i)
            errors.append(error)
        
        #Type 1 Sole proprietor
        if enumeration_type == "NPI-1" and sole_proprietor ==  "YES" and str(d.get('code', '')) not in SOLE_PROPRIETOR_OTHER_NAME_CHOICES:
            error = "other_name %s: %s other_name for sole_proprietor is not in %s." % (i, d.get('code'), SOLE_PROPRIETOR_OTHER_NAME_CHOICES)
            errors.append(error)
        ...
        if enumeration_type == "NPI-1" and sole_proprietor ==  "YES" \
            ...
            and not str(d.get('organization_name', '')):
              error = "other_name %s: Either organization_name or first_name and last_name are required for a sole proprietor." % (i)
              errors.append(error)          
        
        #General
        #supply othertypwe when type is not given.
        if not str(d.get('code', '')) and not str(d.get('othertype', '')):
            error = "other_name %s: othertype must be supplied if other_name type code is blank." % (i)
            errors.append(error)
