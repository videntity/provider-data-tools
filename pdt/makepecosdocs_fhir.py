...
def makepecosdb(database_name="pecos"):
    ...
    try:
        ...
        for bdoc in base_collection.find():
            ...
            d["works_for"] = []
            d["has_providers"] = []
            for rdoc in reassignments_collection.find({"REASGN_BNFT_ENRLMT_ID":
                                                       bdoc['ENRLMT_ID']}):

                ac = base_collection.find({'ENRLMT_ID':
                                           rdoc["RCV_BNFT_ENRLMT_ID"]})
                for a in ac:
                    affiliate = OrderedDict()

                    if a['FIRST_NAME']:
                        affiliate['NAME'] = "%s %s" % (a['FIRST_NAME'],
                                                       a['LAST_NAME'])
                    else:
            ...
            for rdoc in reassignments_collection.find({"RCV_BNFT_ENRLMT_ID":
                                                       bdoc['ENRLMT_ID']}):

                ac = base_collection.find({'ENRLMT_ID':
                                           rdoc["REASGN_BNFT_ENRLMT_ID"]})
                for a in ac:
                    affiliate = OrderedDict()
                    if a['FIRST_NAME']:
                        affiliate['NAME'] = "%s %s" % (a['FIRST_NAME'],
